# ticket_info['fpd_papy_name'].split("-")[1]
ticket_info = {
    "_id": "5d258607f9c8e56192c77e5e",
    "active_epe": [
        "madhun"
    ],
    "base_papy_name": "17-14466",
    "fpd_papy_name": "17-14466-01",
    "software_type": "FIRMWARE",
    "fpd_name": "FAI_TEST_1",
    "mfg_implementation": "Cut in",
    "fpd_status": "Approved",
    "fpd_ver": "v1.0",
    "user_id": "madhun",
    "initiator_name": "Madhu Nettem",
    "current_phase": "Testing",
    "workflow": {
        "unit_test_logs": [
            {
                "testplanname": "as",
                "urllink": "fdsa",
                "tests": "SWDE",
                "testowner": "madhun",
                "formatted": "07-15-2019",
                "comments": "",
                "label": "Unit Testing",
                "eta": {
                    "label": "",
                    "value": {
                        "date": {
                            "year": 2019,
                            "day": 15,
                            "month": 7
                        }
                    },
                    "dateFormat": "yyyy-mm-dd"
                },
                "test_files": [
                    {
                        "uploaded_date_utc": "2019-07-10T06:31:43.971Z",
                        "file_upload": True,
                        "file_id": "626c058aa2dc11e993230050",
                        "file_size": "0.047 KB",
                        "file_location": "ftp://pcam-dev-app-01:2121/DevSCFtpServer/res/home/Software/FPD/Logs/626c058aa2dc11e993230050fil_upload.bin",
                        "file_name": "fil_upload.bin",
                        "uploaded_by": "madhun",
                        "is_active": "Y",
                        "uploaded_date": "07/10/2019 T02:31:43",
                        "file_md5": "eba33de1ce09a35eba54b5fd8140db4e"
                    }
                ],
                "test_comments": "",
                "test_status": "pass",
                "updated_date": "2019-07-10T06:31:46.684Z"
            }
        ],
        "workflow_id": 1,
        "edvt_test_logs": [
            {
                "testplanname": "fsa",
                "urllink": "saf",
                "tests": "EDVT",
                "testowner": "Guest",
                "formatted": "07-15-2019",
                "comments": "",
                "label": "Qualification EDVT/DT",
                "eta": {
                    "label": "",
                    "value": {
                        "date": {
                            "year": 2019,
                            "day": 15,
                            "month": 7
                        }
                    },
                    "dateFormat": "yyyy-mm-dd"
                }
            }
        ],
        "source_code": [],
        "ios_data": [],
        "fpd_images": [
            {
                "uploaded_date_utc": "2019-07-10T06:30:52.613Z",
                "file_name": "fil_upload.bit",
                "file_size": "0.047 KB",
                "is_active": "Y",
                "uploaded_date": "07/10/2019 T02:30:52",
                "file_md5": "eba33de1ce09a35eba54b5fd8140db4e",
                "file_id": "4335e5e6a2dc11e992910050",
                "uploaded_by": "madhun",
                "size": 48.0,
                "extension": "bit",
                "file_location": "ftp://pcam-dev-app-01:2121/DevSCFtpServer/res/home/Software/FPD/4335e5e6a2dc11e992910050fil_upload.bit"
            },
            {
                "file_name": "FW%3a_FPD_Code_Setup.zip",
                "file_size": "212.00 KB",
                "is_active": "Y",
                "uploaded_date": "08/13/2019 T11:51:08",
                "file_md5": "7a64b457dfe048f0a9efb12817aa8ef8",
                "file_id": "7fad27c0bd9211e9883a185e",
                "uploaded_by": "ramaraj",
                "size": 217092.0,
                "extension": "zip",
                "file_location": "ftp://pcam-dev-app-01:2121/DevSCFtpServer/res/home/Software/FPD/7fad27c0bd9211e9883a185eFW%3a_FPD_Code_Setup.zip"
            },
            {
                "file_name": "FW%3a_FPD_Code_Setup.zip",
                "file_size": "212.00 KB",
                "is_active": "Y",
                "uploaded_date": "08/13/2019 T12:01:22",
                "file_md5": "7a64b457dfe048f0a9efb12817aa8ef8",
                "file_id": "ed7b6540bd9311e99414185e",
                "uploaded_by": "ramaraj",
                "size": 217092.0,
                "extension": "zip",
                "file_location": "ftp://pcam-dev-app-01:2121/DevSCFtpServer/res/home/Software/FPD/ed7b6540bd9311e99414185eFW%3a_FPD_Code_Setup.zip",
                "zip_files": [
                    "FPD Scope Environment.docx"
                ]
            },
            {
                "file_name": "README.md",
                "file_size": "1.26 KB",
                "is_active": "Y",
                "uploaded_date": "08/13/2019 T12:07:40",
                "file_md5": "c062cabd1ed6863492a58bae71fdfae2",
                "file_id": "d6863580bd9411e98c13185e",
                "uploaded_by": "ramaraj",
                "size": 1291.0,
                "extension": "md",
                "file_location": "ftp://pcam-dev-app-01:2121/DevSCFtpServer/res/home/Software/FPD/d6863580bd9411e98c13185eREADME.md",
                "zip_files": []
            }
        ]
    },
    "created_by": "madhun",
    "engineering_manager": "Anthony",
    "details": {
        "sw_backword_compatability_comments": "",
        "requested_eta": "2019-7-15",
        "hw_backword_compatability": "Y",
        "critical_warnings": False,
        "reference": "U7",
        "optional_test": [],
        "bug_fixes": "N",
        "board_version": "73-18247-03",
        "diag_backword_compatability_comments": "",
        "change_in_register_spec": "N",
        "cardname": "PCA,MBRD,Celeborn,ASR900 16 popy GE C-SFP + 1 popy SFP+ IM with MACsec",
        "notification_period": "7",
        "cpn_raw_name": "15-13375-01",
        "cdc_review": "",
        "priority": "S3",
        "diag_backword_compatability": "Y",
        "cdets_number": [
            {
                "priority": "S3",
                "type": "Bug Fixes",
                "number": "212"
            }
        ],
        "sw_and_diags_approved": "N",
        "known_issues": "",
        "unconstrained_repopy": True,
        "clocking_constraints": False,
        "description": "IC-LIN,VREG,QFN40,TPS40422,DUAL DC/DC,PMBUS SYNCHRONOUS BUCK CNTLR,Pb-FREE",
        "location_map_and_xml": False,
        "papy17_ver": "IC-Programmed Device,VREG,QFN40,TPS40422,DUAL DC/DC,PMBUS SYNCHRONOUS BUCK CNTLR,Pb-FREE, Coleborn,U7",
        "unit_testing_details": "",
        "hw_backword_compatability_comments": "",
        "summary": "212",
        "sw_backword_compatability": "Y"
    },
    "revision": "-01",
    "skipped": [
        "Qualification EDVT/DT",
        "Qualification QA"
    ],
    "accessible_to": [],
    "component": "17-14466-01",
    "base_papy_version": "01",
    "version_assigned_by_PE": "17-14466-01",
    "last_updated_by": "ramaraj",
    "build_number": "32",
    "progress_step": 1,
    "initiator": "madhun",
    "fpd_hierarchy": {
        "project_name": "Celeborn",
        "item_type": "TAN",
        "item_name": "68-100995-01",
        "bu": "PABU",
        "assembly_name": "68-100995-01",
        "pf": "ASR900"
    },
    "uploader_name": "Madhu Nettem",
    "release_date": "2019-07-10T00:00:00.000Z",
    "new_ver": False,
    "offset_changed": False,
    "default_accessible_to": [
        "madhun"
    ],
    "created_date": "2019-07-10T06:30:31.028Z",
    "fpd_type": "HW",
    "epe": [
        "madhun"
    ],
    "init_approver": [
        {
            "name": "",
            "roles": "epe/Initiator/Creator",
            "cecid": "madhun",
            "stages": [
                {
                    "mandatory": False,
                    "enabled": True,
                    "name": "Local Testing"
                },
                {
                    "mandatory": True,
                    "enabled": True,
                    "name": "Unit Testing"
                },
                {
                    "mandatory": False,
                    "enabled": True,
                    "name": "Dev Testing"
                },
                {
                    "mandatory": True,
                    "enabled": True,
                    "name": "QA Testing"
                },
                {
                    "mandatory": True,
                    "enabled": True,
                    "name": "Production Release"
                }
            ],
            "type": "",
            "deletable": False
        }
    ],
    "fpd_id": "5d258607f9c8e56192c77e5e",
    "last_approver_status": "Approved",
    "correlationId": "5d258697f9c8e56193bf72ce"
}

for key, val in ticket_info.items():
    print(key, " **** ", val)


print("Dictionary :", ticket_info['init_approver'][0]['stages'][3]['name'])
print("------Complex items------")
# 'workflow' 'details' 'init_approver'
print("Ticket number :: ", ticket_info['details']['cdets_number'][0]['number'])
print("Workflow : ", ticket_info['workflow'])
print("Details  : ", ticket_info['details'])
print("Init appr: ", ticket_info['init_approver'])
print("Init appr: ", ticket_info['init_approver'][0]['stages'][2]['name'])


# 1. Get total items in ticket_info dictionary
print("------Total Items-------")
count = 0
for key, value in ticket_info.items():
    print(key, "   -->  ", value)
    count += 1
print("Total Items : ", count)

print("--------------------------------")
# IMPLEMENT BELOW ASSIGNMENTS

print("DAY---------- ", ticket_info['workflow']['unit_test_logs'][0]['eta']['value']['date']['day'])
print("Fpd Images: ", ticket_info['workflow']['fpd_images'])
print("Fpd Images: ", ticket_info['workflow']['fpd_images'][0]['file_location'])
'''
# Get workflow
# Get details 
# Get init_apporver 
        - All records
        - Record whose status is Unit Testing
        - Count stages and display only stages  
# Get fpd_hierarchy assembly_name

'''
print(ticket_info['details']['cdets_number'][0]['number'])
print(ticket_info['details'])
print(ticket_info['details']['cdets_number'])
print(ticket_info['details']['cdets_number'][0])

print("Testing Phase : ", ticket_info["init_approver"][0]["stages"][2]["name"])
print("Testing Phase : ", ticket_info["init_approver"])
print("--------------------------------------")
for key,val in ticket_info["init_approver"][0].items():
    print(key,val)
    if key == 'stages':
        for each in val:
            print(each)
