...
print(x+100)     XXX ---> print(10+100)
 ...
 A  <--------    B     <---------    C
m1(x)             def m2(func,val):              def m3():
                      res = func(val)                m2(m1,10)
                      return res

m2 can call m1 function
m3 can't call m1 function    m1(10) : m1 (10)
...
def  m3():
    #  m1(10)  # Cant directly call m1 function
    output = m2(m1, 10) # # I will take help of m2 to call m1 function
...
X                Y                       Z        
Harsha   <--   Vignesh             <--  Sekhar                 
               10K
          --->                     --> 

foo(bar)           medium(f_name, var)  <==    get_data()       1. foo  2. 10
...
def foo(bar):  # Harsha
    return bar + 1
def mediator(foo_func, val):  # Vignesh
    output = foo_func(val)  # foo(100)
    return output
def get_daata():  # Sekhar
